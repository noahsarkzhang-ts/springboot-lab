 ...
 *
 * @author zhangxt
 * @date 2022/04/11 09:38
 **/
public class CanalOperation {

    protected final static Logger logger = LoggerFactory.getLogger(CanalOperation.class);
    ...
    protected void printEntry(List<Entry> entrys) {
        logger.info("size of entries:{}", entrys.size());

        for (Entry entry : entrys) {

            logger.info("----------------> entry: {}", entry.getEntryType());
            long executeTime = entry.getHeader().getExecuteTime();
            long delayTime = new Date().getTime() - executeTime;
            Date date = new Date(entry.getHeader().getExecuteTime());
            SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");

            final Object[] params = {entry.getHeader().getLogfileName(),
            ...
            if (entry.getEntryType() == EntryType.TRANSACTIONBEGIN || entry.getEntryType() == EntryType.TRANSACTIONEND) {
                ...
                if (entry.getEntryType() == EntryType.TRANSACTIONBEGIN) {
                    ...
                    logger.info(transaction_format, params);
            ...
            if (entry.getEntryType() == EntryType.ROWDATA) {
                ...
                try {
                    ...
                    throw new RuntimeException("parse event has an error , data:" + entry.toString(), ex);
                }

                EventType eventType = rowChange.getEventType();

                final Object[] rowParams = {entry.getHeader().getLogfileName(),
                        String.valueOf(entry.getHeader().getLogfileOffset()), entry.getHeader().getSchemaName(),
                        entry.getHeader().getTableName(), eventType,
                        String.valueOf(entry.getHeader().getExecuteTime()), simpleDateFormat.format(date),
                        entry.getHeader().getGtid(), String.valueOf(delayTime)};
                logger.info(row_format, rowParams);
